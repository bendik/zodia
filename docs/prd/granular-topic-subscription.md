...
- Does `p2panda_net::sync::SyncHandle` actually release its topic subscription on `Drop`, or does `unsubscribe` need an explicit async teardown call? Spike this against the current `p2panda-net` version before committing to the `ZodiaSyncNode` API shape above.
- Is there a per-topic/per-log resource cost in `p2panda-net`'s gossip/discovery layer (e.g. per-topic address-book bookkeeping) that makes "always-subscribed to ~dozens of own-chart keys" meaningfully heavier than today's single topic? If so, the always-subscribed set may need a cap, or the always-subscribed keys may need to share a *single* topic (a "my chart" topic distinct from arbitrary browsing topics) rather than one topic each — worth deciding with real measurements, not guessed.
- Should the unsubscribe grace period be per-key or global (one timer that, when it fires, sweeps all not-recently-visited keys at once)? Per-key is simpler to reason about; a global sweep is cheaper if the timer mechanism itself has non-trivial overhead per instance. Decide once the `SyncHandle` teardown cost from the first open question is known.
