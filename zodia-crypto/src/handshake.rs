//! X3DH-style key agreement for the Tier-1 mutual exchange.
//!
//! Both parties include a static X25519 prekey and a fresh ephemeral key in
//! their Tier1Blob.  After the mutual exchange, each side performs three DH
...
//!
//! The session key then seeds the `MessageRatchet` for all subsequent
//! tier messages (WebRTC signaling, identity reveal).

use x25519_dalek::{PublicKey as X25519Pub, ReusableSecret, StaticSecret};
use rand_core::OsRng;

/// The X25519 key material included in a `Tier1Blob`.
...
impl LocalPrekeys {
    /// Generate a fresh set of X25519 prekeys for a Tier-1 exchange.
...
/// Peer's public prekey material received in their `Tier1Blob`.
#[derive(Debug, Clone)]
